...
<template>
  ...
  <div :style="panorama && adsData.length ? { backgroundImage: `url(${adsData[currentSliderIndex].src})` } : {}">
          <!-- Img -->
      ...
      <div :class="contentClass">
        <div class="py-2">
          <span :class="panorama ? 'bg-black py-1 px-2' : '' " class="text-xs text-slate-400 capitalize">{{ newsType }}</span>
          <h1
            :class="[contentClass ? 'text-3xl font-normal' : '' , panorama ? 'bg-black px-2': '']"
      ...
      <div :class="contentClass">
        <div>
          <p class="text-sm" :class="panorama ? 'bg-black  px-2' : '' ">
            {{ description }}
            <span v-show="ago" class="text-slate-500">- {{ ago }} ago</span>
          </p>
        </div>
      </div>
      <!-- Creator -->
      <div v-show="creatorShow" class="flex items-center py-5">
        ...
        <div class="flex flex-col">
          ...
          <span class="text-xs text-slate-400">{{ creatorNickname }}</span>
        </div>
      </div>
  </div>
</template>